##make tables of significant associations with proportion of reads aligned to CARD protein homolog database (antibiotic resistance genes)
...
for(i in 1:length(taxaLevels)) {
  lev = taxaLevels[i]
  print(lev)
  table = read.table(paste("cardsAssoc_ProHomolog_propMapped_v_16S_", lev, "_pValues.txt", sep=""),
                     header=T, sep="\t")
  sig = table[table$adjLMp < alpha,]
  print(nrow(sig))
  if(nrow(sig) > 0) {
    sig = sig[order(sig$pLM),]
    fulltax = sapply(sig$namesOther, get16sTaxonomy, lev, USE.NAMES = F)
...
table = read.table("cardsAssoc_ProHomolog_propMapped_v_16S_otu_pValues.txt",
                   header=T, sep="\t")
sig = table[table$adjLMp < alpha,]
print(nrow(sig))
sig = sig[order(sig$pLM),]
sig$description = gsub("_", "__", sig$description)
...
df = data.frame(taxaLevel=rep("OTU", nrow(sig)), 
                ...
                spearman = sig$r)
supptable = rbind(supptable, df)
write.table(supptable, "abxAssocTable_16S.txt", sep="\t", row.names = F, col.names = T, quote=F)

#####metabolon
table = read.table("cardsAssoc_ProHomolog_propMapped_v_metabolon_pValues.txt",
                   header=T, sep="\t")
sig = table[table$adjLMp < alpha,] #no hits so don't do anything
print(nrow(sig))

#####metadata
table = read.table("cardsAssoc_ProHomolog_propMapped_v_metadata_pValues.txt",
                   header=T, sep="\t")
sig = table[table$adjLMp < alpha,] #no hits so don't do anything
print(nrow(sig))
...
for(i in 1:length(taxaLevels)) {
  lev = taxaLevels[i]
  print(lev)
  table = read.table(paste("cardsAssoc_ProHomolog_propMapped_v_minikraken_", lev, "_pValues.txt", sep=""),
                     header=T, sep="\t")
...
write.table(supptable, "abxAssocTable_minikraken.txt", sep="\t", row.names = F, col.names = T, quote=F)
...
for(i in 1:length(levels)) {
  lev = levels[i]
  print(lev)
  table = read.table(paste("cardsAssoc_ProHomolog_propMapped_v_humann_", lev, "_pValues.txt", sep=""),
                     header=T, sep="\t")
  sig = table[table$adjLMp < alpha,]
  print(nrow(sig))
  if(nrow(sig) > 0) {
...
write.table(supptable, "abxAssocTable_humann.txt", sep="\t", row.names = F, col.names = T, quote=F)
